    ...
    <div className="modal-overlay" role="dialog" aria-modal="true" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="modal favorites-modal">
        <div className="modal__header">
          <h2 className="modal__title">{t('favorites.title')}</h2>
...
function UserRow({ user, onOpenProfile }: { user: AuthUser; onOpenProfile: (u: string) => void }) {
  return (
    <button
      className="favorites-modal__user-btn"
      onClick={() => { if (user.username) onOpenProfile(user.username); }}
      disabled={!user.username}
    >
      {user.avatar
        ? <img src={user.avatar} alt={user.name} className="favorites-modal__avatar" referrerPolicy="no-referrer" />
      ...
      <span className="favorites-modal__user-info">
        ...
        {user.username && <span className="favorites-modal__username">@{user.username}</span>}
      </span>
    </button>
  );
}
